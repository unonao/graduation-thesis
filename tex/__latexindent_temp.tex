...
For accurate computation, $n_v$ should approaches infinity. However  using large $n_v$ is expensive. For practical purposes, we can use small $n_v$. The convergence rate of such estimator was studied by Avron and Toledo\cite{avron2011randomized}. Hence, we only need to compute the scalars of $\mathbf{z}_i^{T}f(\mathbf{A})\mathbf{z}_i$ which can be estimated by using stochastic Lanczos quadrature.
